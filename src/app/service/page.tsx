...
const services = [
  { name: "Website", description: "Landing page, company profile, dll." },
  {
    ...
    description: "Undangan nikah, ulang tahun, dll.",
  },
...
const ServicePage: React.FC = () => {
  ...
  const handleChanges = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!value) setWhatsappError("Masukkan no Whatsapp diawali angka 0");
    ...
    >
      ...
      <div className="mt-10 w-full h-full max-w-4xl relative">
        <section
          ...
          hover:before:opacity-100 hover:before:bg-[radial-gradient(circle_at_var(--mouse-x)_var(--mouse-y),_rgba(34,197,94,0.2)_0%,_transparent_25%)]"
          data-aos="fade-up"
          onMouseMove={(e) => {
        ...
        <div className="absolute lg:w-full w-60 -z-1 inset-0 bg-gradient-to-r from-green-300 to-blue-300 dark:from-green-400 dark:to-blue-400 rounded-3xl transform -rotate-6 scale-105 opacity-20"></div>
      </div>

      {/* Modal Hero UI: Detail kosong */}
      ...
      {/* Modal Hero UI: WhatsApp */}
      <Modal
        isOpen={isModalVisiblewa}
        onClose={() => setIsModalVisiblewa(false)}
      >
        <ModalHeader>
          <h2 className="text-2xl font-bold text-center text-white">
        ...
        <ModalBody className="flex flex-col space-y-3">
          ...
          />
          {whatsappError && (
            <p className="text-red-400 text-sm">{whatsappError}</p>
          )}
        </ModalBody>
        <ModalFooter className="space-x-2 justify-end">
          <Button onClick={() => setIsModalVisiblewa(false)} variant="solid">
            Batal
          </Button>
          <Button
